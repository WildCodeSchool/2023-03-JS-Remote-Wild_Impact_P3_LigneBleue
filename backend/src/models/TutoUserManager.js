...
class TutoUserManager extends AbstractManager {
  constructor() {
  ...
  find(id) {
    return this.database.query(
      `select t.id,f.title,t.name, t.target, t.explanation, i.src,i.alt, q.title,  quest.content, a.content from formations JSON_ARRAYAGG(JSON_OBJECT('id', q.id, 'title', q.title)) as quizz
      as f inner join tutorials as t on t.formation_id=f.id 
      inner join images as i on t.image_id=i.id 
      inner join quizz as q on t.quizz_id=q.id 
      inner join questions as quest on quest.quizz_id=q.id 
      inner join answers as a on a.question_id=quest.id  where t.id = ?`,
      [id]
    );
  }
...
module.exports = TutoUserManager;
