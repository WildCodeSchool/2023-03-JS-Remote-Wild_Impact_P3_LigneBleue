...
function TutoProgress() {
  ...
  const [validatedTuto, setValidatedTuto] = useState(0);
  const { fid } = useParams();
  ...
  const getTutorialsList = async () => {
    try {
      ...
      setList(tutoList);
      const validated = tutoList.filter((el) => el.validated === 1);
      setValidatedTuto(validated.length);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  return (
    <>
      <h2 className="text-center text-blue text-xl mt-4 font-bold">
        Vous avez validé {validatedTuto} tutos sur 8 ! Continuez!{" "}
      </h2>
      <section>
